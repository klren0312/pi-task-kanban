...
- `pi.sendUserMessage`（ExtensionAPI 上）是同步 void 函数，agent 流式运行期间不带 `deliverAs` 调用会直接 throw：发送前用 `ctx.isIdle()` 守卫；忙碌时用 `{ deliverAs: "followUp" }`；发送失败的 catch 里必须把任务回退到合法状态并持久化（参考 tryDispatch 的发送失败回退）。
